...
uint256 hashGenesisBlock("0x320ee529d52c0fc22ae3ad8877665c04f339e309d7dedcf57ff4ecf9f905b460");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 2bacco: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "2bacco Signed Message:\n";
...
{
    // 2bacco: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 81454500 * 12  / 100 / 512;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // 2bacco: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // 2bacco: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // 2bacco: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x320ee529d52c0fc22ae3ad8877665c04f339e309d7dedcf57ff4ecf9f905b460");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1429638554;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085875315;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("2baccoMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("2baccoMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running 2bacco with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
